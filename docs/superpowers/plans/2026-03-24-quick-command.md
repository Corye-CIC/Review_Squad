...
**Goal:** Create a `/quick` command that runs selected Review Squad agents directly on a short task description, bypassing the full lifecycle.

**Architecture:** A single command file (`commands/quick.md`) that parses `$ARGUMENTS` right-to-left to detect agent tokens, routes through one of three execution paths (PM Cory routing / self-select pre-flight / explicit modes), and optionally appends Nando synthesis. The setup guide gets a matching inline section. No new agent files needed — all existing mode-suffixed agents are reused.

**Tech Stack:** Markdown command file (Claude Code slash command format), existing agents: `pm-cory-early`, `{name}-{mode}.md` pattern, `AskUserQuestion` tool for confirmation prompts.

**Spec:** `docs/superpowers/specs/2026-03-24-quick-command-design.md`

---

## Chunk 1: `commands/quick.md`

### Task 1: Scaffold — frontmatter, init source, objective, context

**Files:**
- Create: `commands/quick.md`

- [ ] **Step 1: Write the file with frontmatter + init-session source line + objective + context**
...
```bash
head -20 commands/quick.md
```
Expected: frontmatter block with `name: quick`, all 8 allowed-tools listed.

---

### Task 2: Argument parsing section

**Files:**
- Modify: `commands/quick.md`

- [ ] **Step 1: Append the `<parsing>` section**

Append to `commands/quick.md`:
...
- [ ] **Step 2: Verify section appended cleanly**

```bash
grep -n "Argument Parsing\|AGENT_LIST\|HAS_NANDO" commands/quick.md
```
Expected: lines found for all three terms.

---

### Task 3: Process — Path 1 (PM Cory routing)

**Files:**
- Modify: `commands/quick.md`

- [ ] **Step 1: Append `<process>` opening and Path 1**
...
- [ ] **Step 2: Spot-check the agent subagent_type pattern is correct**

```bash
grep "subagent_type\|father-christmas\|stevey-boy-choi" commands/quick.md
```
Expected: all three terms present.

---

### Task 4: Process — Path 2 (self-select pre-flight)

**Files:**
- Modify: `commands/quick.md`

- [ ] **Step 1: Append Path 2**
...
- [ ] **Step 2: Verify all Path 2 sub-steps are present**

```bash
grep -n "Pre-flight\|All-low\|Confirm kept\|Edit flow" commands/quick.md
```
Expected: all four section headings found.

---

### Task 5: Process — Path 3, +nando, output format

**Files:**
- Modify: `commands/quick.md`

- [ ] **Step 1: Append Path 3, +nando synthesis, and output format**
...
- [ ] **Step 2: Verify process section closes properly**

```bash
grep -n "</process>\|</parsing>\|<success_criteria>" commands/quick.md
```
Expected: `</process>` and `</parsing>` present.

---

### Task 6: Success criteria + final verification

**Files:**
- Modify: `commands/quick.md`

- [ ] **Step 1: Append success_criteria**
...
```bash
grep -n "^---\|<objective>\|<context>\|<parsing>\|<process>\|<success_criteria>" commands/quick.md
```
Expected: all 6 structural markers present.

- [ ] **Step 3: Spec compliance check — verify each path is represented**

```bash
grep -n "Path 1\|Path 2\|Path 3\|HAS_NANDO\|AskUserQuestion\|all-low" commands/quick.md
```
...
git commit -m "feat: add /quick command — ad-hoc agent dispatch with PM Cory routing and self-select pre-flight"
```

---

## Chunk 2: Setup Guide Update

### Task 7: Add `/quick` inline section to the setup guide

**Files:**
- Modify: `review-squad-setup - v3.md`

The setup guide documents all commands inline so users can copy them to a new machine. `/quick` needs a `#### Command: /quick` section and a mention in the Quick Start command list.

- [ ] **Step 1: Find the insertion point — after `/ship` section, before `#### GSD Command`**

```bash
grep -n "#### Command: /ship\|#### GSD Command" "review-squad-setup - v3.md"
```
Expected: two line numbers. The `/quick` section goes between them.

- [ ] **Step 2: Confirm exact insertion boundary**

```bash
grep -n "All artifacts written to .review-squad" "review-squad-setup - v3.md"
```
Note the line number. The closing ` ``` ` of the ship block is 2 lines below it. That closing ` ``` ` is the last line of the ship section — the `/quick` section inserts immediately after it (before the blank line and `#### GSD Command`).

- [ ] **Step 4: Read `commands/quick.md` in full, then insert the `/quick` section**

Re-read `commands/quick.md` to get its current, complete contents. Use the Edit tool to insert the following block at the boundary identified in Step 3 (after the ship block's closing ` ``` `, before `#### GSD Command`):
...
```markdown
{verbatim contents of commands/quick.md}
```

````

The outer fence is 4 backticks (` ```` `). This prevents the inner triple-backtick fences inside `commands/quick.md` (` ```bash `, plain ` ``` `) from terminating the outer block prematurely — they are file content, not structural fences.

- [ ] **Step 5: Verify insertion structure**

```bash
grep -n "#### Command: /quick\|#### GSD Command" "review-squad-setup - v3.md"
```
Expected: `#### Command: /quick` appears on a line before `#### GSD Command`.

- [ ] **Step 6: Update Quick Start command list**

Find the Quick Start section (around line 235–244). Make two edits:
1. Change `# 4. Create the 9 command files` → `# 4. Create the 10 command files`
2. Add `#    ~/.claude/commands/quick.md` to the list (after `audit.md`, before `gsd/review.md`)
3. Change `ls ~/.claude/commands/*.md        # Should list 8 files` → `# Should list 9 files` (root commands only; gsd/review.md is separate)

- [ ] **Step 7: Verify guide counts**

```bash
grep -n "command files\|Should list\|/quick" "review-squad-setup - v3.md" | head -15
```
Expected: "10 command files", "Should list 9 files", and `/quick` in the list.

- [ ] **Step 8: Commit**

```bash
git add "review-squad-setup - v3.md"
git commit -m "docs: add /quick command inline section to setup guide"
```

---

## Validation Checklist (complete before marking done)

- [ ] `commands/quick.md` exists and has all structural tags (`<parsing>`, `<process>`, `<success_criteria>`)
- [ ] All three execution paths documented in process section
- [ ] +nando synthesis section present
- [ ] AskUserQuestion used for all confirmation prompts (never assumes y)
- [ ] Setup guide Quick Start lists 10 command files
- [ ] Setup guide has `#### Command: /quick` section matching the actual file
- [ ] Both commits clean (`git log --oneline -3`)
